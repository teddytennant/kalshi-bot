"""TUI dashboard for kalshi-bot using Textual."""
...
from kalshi_bot.strategy import MeanReversionStrategy
...
class DashboardApp(App):
    """Textual TUI dashboard for kalshi-bot."""
    ...
    def on_mount(self) -> None:
        ...
        pos_table.add_columns("Ticker", "Side", "Qty", "Avg", "Unrealized")

        scan_table = self.query_one("#scanner", DataTable)
        scan_table.add_columns("Ticker", "Bid", "Ask", "Signal")
    ...
    def _trading_loop(self) -> None:
        from kalshi_bot.runner import run_cycle
        ...
        while not self._should_stop.is_set():
            ...
            # Sleep in 100ms increments for responsive pause
    ...
    def _refresh_ui(self) -> None:
        ...
        for event in events:
            ...
            if event.event_type == EventType.CYCLE_START:
                ...
                ticker = d.get("ticker", "?")
                self._market_prices[ticker] = (
                    d.get("yes_bid", "0"),
                    d.get("yes_ask", "0"),
                )

        # Refresh scanner table from latest scan events
        if events:
            scanned = [
    ...
    def _refresh_positions(self) -> None:
        ...
        for (ticker, side), pos in self._portfolio.positions.items():
            ...
            elif prices and side == Side.NO:
                current = Decimal("1.00") - Decimal(prices[1])  # 1 - yes_ask
                unrealized = (current - pos.avg_price) * pos.quantity
                unrl_str = f"{'+' if unrealized >= 0 else ''}{unrealized:.2f}"
            else:
                unrl_str = "--"
