...
from kalshi_bot.models import Fill, Position, Side
...
class Portfolio:
    def __init__(self, initial_balance: Decimal = Decimal("10000.00")):
        ...
        # Key: (ticker, Side) -> mutable position data
        self._positions: dict[tuple[str, Side], _MutablePosition] = {}

    @property
    def positions(self) -> dict[tuple[str, Side], Position]:
        return {
            k: Position(ticker=k[0], side=k[1], quantity=v.quantity, avg_price=v.avg_price)
    ...
    def get_position(self, ticker: str, side: Side) -> Optional[Position]:
        key = (ticker, side)
        pos = self._positions.get(key)
        if pos is None:
            return None
        return Position(ticker=ticker, side=side, quantity=pos.quantity, avg_price=pos.avg_price)
    ...
    def settle_market(self, ticker: str, result: str) -> None:
    ...
    def from_dict(cls, data: dict) -> Portfolio:
        ...
        for pos_data in data.get("positions", []):
            key = (pos_data["ticker"], Side(pos_data["side"]))
