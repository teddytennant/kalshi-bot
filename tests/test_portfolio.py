...
from kalshi_bot.models import Fill, Side, Position
...
class TestRecordFill:
    def test_buy_yes_deducts_balance(self):
        p = Portfolio()
    ...
    def test_multiple_fills_update_avg_price(self):
        p = Portfolio()
        fill1 = Fill(ticker="T", side=Side.YES, price=Decimal("0.60"), quantity=10)
        fill2 = Fill(ticker="T", side=Side.YES, price=Decimal("0.70"), quantity=10)
        p.record_fill(fill1)
        p.record_fill(fill2)
        pos = p.get_position("T", Side.YES)
    ...
    def test_buy_no_creates_no_position(self):
        p = Portfolio()
        fill = Fill(ticker="T", side=Side.NO, price=Decimal("0.35"), quantity=5)
        p.record_fill(fill)
        pos = p.get_position("T", Side.NO)
        assert pos is not None
    ...
    def test_yes_and_no_positions_separate(self):
        ...
        p.record_fill(Fill(ticker="T", side=Side.NO, price=Decimal("0.35"), quantity=5))
...
class TestGetPosition:
    ...
    def test_returns_position(self):
        p = Portfolio()
        p.record_fill(Fill(ticker="T", side=Side.YES, price=Decimal("0.50"), quantity=1))
        pos = p.get_position("T", Side.YES)
        assert pos is not None
...
class TestClosePosition:
    def test_close_credits_balance(self):
        ...
        p.record_fill(Fill(ticker="T", side=Side.YES, price=Decimal("0.60"), quantity=10))
        initial = p.balance
...
class TestSettleMarket:
    def test_settle_yes_winner(self):
        ...
        p.settle_market("T", result="yes")
        # YES wins: each contract pays $1.00 = 100 cents
        assert p.balance == balance_before + Decimal("10.00")
        assert p.get_position("T", Side.YES) is None

    def test_settle_no_winner(self):
        p = Portfolio()
        p.record_fill(Fill(ticker="T", side=Side.NO, price=Decimal("0.35"), quantity=10))
        balance_before = p.balance
        p.settle_market("T", result="no")
        assert p.balance == balance_before + Decimal("10.00")
    ...
    def test_settle_both_sides(self):
        ...
        p.record_fill(Fill(ticker="T", side=Side.NO, price=Decimal("0.35"), quantity=5))
        p.settle_market("T", result="yes")
        # YES wins: +10.00, NO loses: +0.00
        # PnL: (1.00-0.60)*10 + (0.00-0.35)*5 = 4.00 - 1.75 = 2.25
...
class TestSerialization:
    ...
    def test_roundtrip(self):
        p = Portfolio()
        p.record_fill(Fill(ticker="T", side=Side.YES, price=Decimal("0.60"), quantity=10))
        p.record_fill(Fill(ticker="X", side=Side.NO, price=Decimal("0.40"), quantity=5))
        d = p.to_dict()
