...
class TestEventBusCursorTracking:
    def test_cursor_is_monotonic(self):
        bus = EventBus()
        cursors = []
        for i in range(5):
            bus.emit(EventType.CYCLE_START, cycle=i)
            _, c = bus.drain_from(cursors[-1] if cursors else 0)
            cursors.append(c)
...
class TestEventBusTrimming:
    ...
    def test_stale_cursor_gets_available_events(self):
        bus = EventBus(max_events=3)
        for i in range(10):
            bus.emit(EventType.CYCLE_START, cycle=i)

        events, cursor = bus.drain_from(2)
        assert len(events) == 3
        assert events[0].data == {"cycle": 7}
